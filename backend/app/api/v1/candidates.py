...
def list_candidates(skip: int = Query(0, ge=0), limit: int = Query(100, ge=1, le=1000),
                   db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    candidates = crud_candidate.get_multi(db, skip=skip, limit=limit)
    total = crud_candidate.get_count(db)
    return {"total": total, "skip": skip, "limit": limit, "items": candidates}

@router.get("/search", response_model=PaginatedResponse[CandidateResponse])
def search_candidates(q: str = Query(..., min_length=2), skip: int = Query(0, ge=0),
...
def unlock_candidate(candidate_id: int, db: Session = Depends(get_db),
                    current_user: User = Depends(get_current_user)):
